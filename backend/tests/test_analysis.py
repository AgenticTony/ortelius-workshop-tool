...
def test_run_analysis_no_ideas(client, sample_session, auth_headers):
    # Auth required + no ideas -> auth passes, then 400 for empty session.
    response = client.post(
        f"/sessions/{sample_session['id']}/analyse",
...
def test_run_analysis_requires_token(client, sample_session):
    # No Authorization header -> 401.
    response = client.post(f"/sessions/{sample_session['id']}/analyse")
    assert response.status_code == 401


def test_run_analysis_wrong_token(client, sample_session):
    response = client.post(
        f"/sessions/{sample_session['id']}/analyse",
...
def test_run_analysis_session_not_found(client):
    response = client.post(
        "/sessions/doesnotexist/analyse",
        headers={"Authorization": "Bearer anything"},
    )
    # 404 (session check precedes token verification in get_facilitator).
    assert response.status_code == 404
...
def test_download_report_requires_token(client, sample_session):
    response = client.get(f"/sessions/{sample_session['id']}/report")
    assert response.status_code == 401


def test_download_report_no_analysis(client, sample_session, auth_headers):
    response = client.get(
        f"/sessions/{sample_session['id']}/report",
        headers=auth_headers(sample_session),
    )
    assert response.status_code == 400
